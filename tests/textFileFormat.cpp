...
bool SdfTextFileFormat::_ReadSdfRcs(SdfLayer* layer,
                                    const std::string& resolvedPath,
                                    bool metadataOnly) const
{
    ...
    char buffer[81] = {0};  // Extra byte for null-terminator
    fin.read(buffer, 80);

    // Convert the buffer to a string
    std::string header(buffer);

    // Check if the header contains "SDF Version 2.0"
    if (header.find("SDF Version 2.0") == std::string::npos) {
        TF_RUNTIME_ERROR("File: %s is not an SDF V2.0 file.", resolvedPath.c_str());
        return false;
    }

    // Continue with the rest of the file reading and parsing
    // ...
    std::cout << "File: " << resolvedPath << " is an SDF V2.0 file." << std::endl;
    ...
    bool metadataOnly) const
{
    ...
    // Try to read sdf RCS file format first.
    if (_ReadSdfRcs(layer, resolvedPath, metadataOnly)){
        return true;
    }
